...
TGraph* get_peak_graph(HistType* hist) {
			...
			for (int i = 0; i < 1; ++i) {
				for (int j = 0; j < N_PADDLES; ++j) {
					...
					if (!hist)
						continue;
					c1->cd(++pad_idx);
					//TProfile *profile = new TProfile(TString::Format("profile_%d_%d_%d",i_hist,i_plane,j), "Profile above line", hist->GetNbinsX(),
					//                                 hist->GetXaxis()->GetXmin(), hist->GetXaxis()->GetXmax());
					//TProfile *profile = hist->ProfileX(TString::Format("profile_%d_%d_%d", i_hist, i_plane, j), 1, -1, "e");
					TGraph* profile = get_peak_graph(hist);
          if (!profile) {
